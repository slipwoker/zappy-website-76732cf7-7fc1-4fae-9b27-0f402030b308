    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            const response = await fetch('https://qaapi.zappy5.com/api/email/contact-form', {
                ...
                body: JSON.stringify({
                    websiteId: '76732cf7-7fc1-4fae-9b27-0f402030b308',
                    name: data.name || '',
                    email: data.email || '',
...
(function() {
  ...
  function getCurrentEcomLanguage() {
    ...
    var stored = localStorage.getItem('zappy_lang');
  ...
  function isCurrentEcomRTL() {
    ...
    return ['he', 'ar', 'iw'].includes(lang) || document.documentElement.dir === 'rtl';
  }
  ...
  try {
    ...
    var dropdownItemsHtml = topLevel.map(function(cat) {
      ...
      if (children.length === 0) {
        return '<li data-category-id="' + cat.id + '" data-category-slug="' + (cat.slug || '') + '"><a href="' + catUrl(cat) + '" dir="auto">' + cat.name + '</a></li>';
      }
      var items = '<li class="zappy-nav-parent" data-category-id="' + cat.id + '" data-category-slug="' + (cat.slug || '') + '"><a href="' + catUrl(cat) + '" dir="auto">' + cat.name + '</a></li>';
      children.forEach(function(child) {
        items += '<li class="zappy-nav-child" data-category-id="' + child.id + '" data-category-slug="' + (child.slug || '') + '"><a href="' + catUrl(child) + '" dir="auto">' + child.name + '</a></li>';
      });
      return items;
    }).join('');
    ...
    var barItemsHtml = topLevel.map(function(cat) {
      var children = childrenMap[cat.id] || [];
      if (children.length === 0) {
        return '<a href="' + catUrl(cat) + '" class="catalog-menu-item" data-category-id="' + cat.id + '" data-category-slug="' + (cat.slug || '') + '" dir="auto">' + cat.name + '</a>';
      }
      var subLinks = children.map(function(child) {
        return '<a href="' + catUrl(child) + '" class="catalog-menu-item" data-category-id="' + child.id + '" data-category-slug="' + (child.slug || '') + '" dir="auto">' + child.name + '</a>';
      }).join('');
      return '<div class="catalog-menu-parent" data-category-id="' + cat.id + '" data-category-slug="' + (cat.slug || '') + '">' +
        '<a href="' + catUrl(cat) + '" class="catalog-menu-item catalog-menu-trigger" dir="auto">' + cat.name + ' ' + chevronSvg + '</a>' +
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function updateStaticEcommerceUI() {
    ...
    if (termsLink) termsLink.textContent = getEcomText('termsAndConditions', 'Terms and Conditions');
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '76732cf7-7fc1-4fae-9b27-0f402030b308';
...
(function() {
  ...
  (function seedLanguageFromUrl() {
    var urlLang = getQueryLang() || getPathLang();
    if (!urlLang) return;
